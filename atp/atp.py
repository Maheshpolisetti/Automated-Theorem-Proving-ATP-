...
#     # Manual Test Case: EmptySet Axiom check
#     prover = TheoremProver(include_set_axioms=True)
    
#     test_kb = ["∀ax_v1 ¬In(ax_v1, EmptySet)"] # Simple KB
#     test_goal = "forall A forall B forall C (Equals(Union(A, Intersect(B, C)), Intersect(Union(A, B), Union(A, C))))" # Simple Goal
    
#     print("RUNNING MANUAL CHECK...")
#     try:
#         success, proof_log, all_clauses = prover.prove(test_kb, test_goal)
#         print(f"\nFINAL RESULT: {'PROVED' if success else 'NOT PROVED'}")
#     except Exception as e:
#         print(f"\nMANUAL CHECK FAILED WITH ERROR: {e}")
#         import traceback
#         traceback.print_exc()